...
<script type="text/javascript">
				...
				.done(
						function(list) {
							...
							for (var i = 0; i < file.length; i++) {
								...
								tub += "<label id ='upload-name-"+ file[i].fileBno + "'onclick='OnDelBtn("+ file[i].fileBno + ")' style='padding: 14px 10px 11px 10px; border-radius: 4px; background: #570df6; color: white;'>파일 삭제</label>"
							}
							;
	...
	$(document).on('change', '#file', function() {
		...
		if (total < 3){
			...
			newFile = "<input class='form-control' id='upload-new-name-"+ filesArray.length + "' style='border-radius: 0px 4px 4px 0px !important; width: 88%; display: inline; margin-bottom:5px;' placeholder='첨부 파일' value='" + $("#file").get(0).files[0].name + "'readonly>";
			newFile += "<label id ='upload-new-name-"+ filesArray.length + "'onclick='OnDelFile("+ filesArray.length + ")' style='padding: 14px 10px 11px 10px; border-radius: 4px; background: #570df6; color: white;'>파일 삭제</label>"
